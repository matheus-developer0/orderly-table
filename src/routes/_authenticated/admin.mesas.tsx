...
type Table = {
  id: string; number: number; qr_code: string; status: TableStatus;
  order_total?: number; order_count?: number; occupied_since?: string | null;
};

const STATUS_META: Record<TableStatus, { label: string; color: string; dot: string }> = {
    ...
    <>
      ...
      <motion.div initial={{ opacity: 0, scale: 0.96 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.96 }}
        ...
        <div className="flex items-center justify-between mb-5">
          <h2 className="text-lg font-extrabold">Editar Mesa {table.number}</h2>
          <button onClick={onClose} className="rounded-full p-1 hover:bg-muted text-muted-foreground"><X className="h-5 w-5" /></button>
        </div>
        <div className="space-y-1.5">
          <label className="text-xs font-semibold text-muted-foreground uppercase tracking-wider">Novo número</label>
          <input type="number" min={1} autoFocus value={number} onChange={e => setNumber(e.target.value)}
            onKeyDown={e => e.key === "Enter" && void save()} className="input-base" />
        </div>
...
function QRModal({ table, qrUrl, onClose }: { table: Table; qrUrl: string; onClose: () => void }) {
  ...
  const download = () => {
    ...
    a.href = qrUrl; a.download = `mesa-${table.number}.png`; a.click();
  };

  const print = () => {
    const win = window.open("", "_blank");
    if (!win) return;
    win.document.write(`<html><head><title>Mesa ${table.number}</title>
    <style>*{margin:0;padding:0;box-sizing:border-box}body{font-family:sans-serif;display:flex;flex-direction:column;align-items:center;justify-content:center;min-height:100vh;background:#fff;gap:16px}
    img{width:260px;height:260px}h1{font-size:28px;font-weight:900}p{color:#666;font-size:14px;text-align:center}
    .url{font-size:11px;color:#999;max-width:260px;word-break:break-all;text-align:center}</style>
    </head><body>
    <img src="${qrUrl}" />
    <h1>Mesa ${table.number}</h1>
    <p>Escaneie para ver o cardápio e fazer pedidos</p>
  ...
  return (
    <>
      ...
      <motion.div initial={{ opacity: 0, scale: 0.96 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.96 }}
        ...
        <div className="rounded-2xl bg-white p-4 mx-auto w-fit border border-border/30">
          <img src={qrUrl} alt={`QR Mesa ${table.number}`} className="h-52 w-52" />
        </div>
        <p className="mt-3 text-xs text-muted-foreground">Aponte a câmera para acessar o cardápio</p>
        <div className="mt-2 rounded-lg bg-muted px-3 py-1.5 text-[11px] text-muted-foreground font-mono truncate">{mesaUrl}</div>
...
function MesasPage() {
  ...
  const load = useCallback(async () => {
    ...
    // Enrich with order info for occupied tables
  ...
  useEffect(() => {
    if (!restaurant) return;
    const ch = supabase.channel(`tables-${restaurant.id}`)
  ...
  const addTables = async () => {
    ...
    const rows = Array.from({ length: addCount }, (_, i) => ({
      ...
      number: lastNum + i + 1,
      qr_code: `${restaurant.slug}-mesa-${lastNum + i + 1}`,
    }));
      ...
      <div className="border-b border-border bg-card px-6 py-5 space-y-4 shrink-0">
        <div className="flex items-center justify-between gap-4 flex-wrap">
          <div>
            ...
            <p className="text-sm text-muted-foreground mt-0.5">{tables.length} mesas cadastradas</p>
          </div>
          <div className="flex items-center gap-2 flex-wrap">
            ...
            <select value={addCount} onChange={e => setAddCount(Number(e.target.value))}
              className="h-10 rounded-xl border border-border bg-background px-3 text-sm font-medium outline-none cursor-pointer">
        ...
        <div className="flex gap-2 flex-wrap">
          {([["all","Todas"],["free","Livres"],["occupied","Ocupadas"],["bill_requested","Conta pedida"]] as const).map(([key, label]) => (
            <button key={key} onClick={() => setFilterStatus(key)}
              className={cn("flex items-center gap-1.5 rounded-xl border px-3 py-1.5 text-xs font-semibold transition-all",
                filterStatus === key ? "gradient-brand text-primary-foreground border-transparent shadow-brand" : "border-border text-muted-foreground hover:bg-muted")}>
              {key !== "all" && <span className={cn("h-1.5 w-1.5 rounded-full", STATUS_META[key as TableStatus].dot)} />}
              {label}
        ...
        {loading ? (
          ...
          <div className="grid place-items-center py-24 text-center">
            ...
            <p className="text-sm text-muted-foreground">
              {tables.length === 0 ? "Nenhuma mesa criada ainda. Clique em Adicionar." : "Nenhuma mesa com este status."}
            </p>
          </div>
        ) : (
          <div className="grid grid-cols-2 gap-4 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5">
            <AnimatePresence mode="popLayout">
              {filtered.map((t, i) => {
                ...
                return (
                  <motion.div key={t.id} layout initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, scale: 0.9 }}
                    transition={{ delay: Math.min(i, 10) * 0.02 }}
                    className={cn("rounded-2xl border bg-card shadow-card flex flex-col overflow-hidden",
                    ...
                    <div className="p-4 pb-2">
                      ...
                      {t.status !== "free" && t.order_total && (
                        <div className="mt-2">
                          <div className="text-sm font-extrabold text-primary">{fmt(t.order_total)}</div>
                          {t.occupied_since && <div className="text-[11px] text-muted-foreground">{timeAgo(t.occupied_since)} na mesa</div>}
                        </div>
                      )}
                    </div>

                    {/* QR */}
                    <button className="mx-3 mb-2 rounded-xl bg-white p-2 border border-border/30 cursor-pointer"
                      onClick={() => qrs[t.id] && setQrModal(t)}>
                      {qrs[t.id]
                        ? <img src={qrs[t.id]} alt={`QR ${t.number}`} className="h-28 w-full object-contain" />
                    ...
                    <div className="mt-auto grid grid-cols-3 border-t border-border divide-x divide-border">
                      ...
                      {t.status !== "free" ? (
                        <button onClick={() => void freeTable(t)}
                        ...
                        <button onClick={() => qrs[t.id] && setQrModal(t)}
                          className="flex flex-col items-center gap-1 py-2.5 text-[10px] font-medium text-muted-foreground hover:bg-muted transition-colors">
                          <QrIcon className="h-3.5 w-3.5" />Ver QR
                        </button>
                      )}
                      <button onClick={() => void deleteTable(t)}
                        ...
                        title={t.status !== "free" ? "Só é possível excluir mesas livres" : "Excluir"}>
