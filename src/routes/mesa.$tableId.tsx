...
const STATUS_INFO: Record<string,{label:string;desc:string;emoji:string;step:number}> = {
  new:              {label:"Pedido recebido!",  desc:"A cozinha recebeu e vai preparar em breve.", emoji:"🎉",step:0},
  preparing:        {label:"Em preparo",         desc:"Mãos na massa! Seu pedido está sendo feito.",emoji:"👨‍🍳",step:1},
  ready:            {label:"Pronto!",            desc:"O garçom já vem trazer sua comanda.",        emoji:"✅",step:2},
...
function MesaPage() {
  ...
  const [orderHistory,  setOrderHistory] = useState<{id:string;total:number;status:string}[]>([]);
  ...
  const load = useCallback(async () => {
    ...
    const [{ data:r },{ data:prods },{ data:cats }] = await Promise.all([
      ...
      supabase.from("categories").select("id,name,sort_order,image_url").eq("restaurant_id",t.restaurant_id).order("sort_order"),
    ]);
  ...
  useEffect(()=>{
    ...
    const ch = supabase.channel(`track-${lastOrderId}`)
  ...
  const brand = restaurant?.primary_color ?? "#E11D2E";
  ...
  const sendOrder = async () => {
    ...
    setOrderHistory(prev=>[{id:order.id,total:cartTotal,status:"new"},...prev]);
  ...
  const filtered = products.filter(p=>{
    ...
    const mc=!activeCat||p.category_id===activeCat;
    return ms&&mc;
  });
  const featured = products.filter(p=>p.featured);

  if (loading) return <div className="grid min-h-screen place-items-center bg-background"><Loader2 className="h-6 w-6 animate-spin" style={{color:brand}}/></div>;
  ...
  if (screen==="identify") return (
    <div className="min-h-screen bg-background flex flex-col">
      <div className="px-5 pb-14 pt-14 text-white relative overflow-hidden" style={{background:`linear-gradient(135deg, ${brand} 0%, #111 100%)`}}>
      ...
      <div className="flex-1 px-5 py-8 space-y-5 max-w-sm mx-auto w-full">
        ...
        <div className="space-y-1.5">
          ...
          <div className="relative">
            ...
            <input type="tel" value={customerPhone} onChange={e=>setCustomerPhone(e.target.value)} className="input-base pl-9" placeholder="(11) 99999-9999"/>
          </div>
        </div>
        <button onClick={()=>customerName.trim()&&setScreen("menu")} disabled={!customerName.trim()}
          className="flex h-14 w-full items-center justify-center gap-2 rounded-2xl font-extrabold text-white shadow-xl disabled:opacity-40 active:scale-95 transition-transform"
          style={{background:brand}}>
          Ver cardápio <ChevronRight className="h-5 w-5"/>
        </button>
        <p className="text-center text-xs text-muted-foreground">Seus dados são usados apenas para organizar o pedido na mesa.</p>
      </div>
    </div>
  );
  ...
  if (screen==="tracking") {
    ...
    return (
      <div className="min-h-screen bg-background flex flex-col">
        <div className="px-5 pb-10 pt-12 text-white text-center relative overflow-hidden" style={{background:`linear-gradient(135deg, ${brand} 0%, #111 100%)`}}>
          <div className="absolute -left-20 -top-20 h-56 w-56 rounded-full bg-white/10 blur-3xl"/>
          <div className="relative space-y-2">
            <motion.div key={info.emoji} initial={{scale:0.5,opacity:0}} animate={{scale:1,opacity:1}} className="text-5xl">{info.emoji}</motion.div>
            <h1 className="text-2xl font-extrabold">{info.label}</h1>
            <p className="text-sm text-white/70">{info.desc}</p>
            <p className="text-xs text-white/50">Mesa {table.number} · {customerName}</p>
          </div>
        </div>
        ...
        <div className="flex-1 px-5 py-6 space-y-5 max-w-sm mx-auto w-full">
          ...
          {orderStatus!=="cancelled"&&(
            <div className="rounded-2xl border border-border bg-card p-5 shadow-card">
              <div className="flex items-center">
                {STEPS.map((label,i)=>{
                  const done=i<=info.step; const active=i===info.step;
                  return (
                    <div key={label} className="flex items-center flex-1">
                      <div className="flex flex-col items-center">
                        <div className="h-8 w-8 rounded-full grid place-items-center text-xs font-extrabold transition-all"
                          style={done?{background:brand,color:"#fff"}:{background:"var(--muted)",color:"var(--muted-foreground)"}}>
                      ...
                      {i<STEPS.length-1&&(
                        <div className="flex-1 h-0.5 mb-4 mx-1 rounded-full" style={{background:i<info.step?brand:"var(--border)"}}/>
                      )}
                    </div>
                  );
                })}
          ...
          <div className="space-y-2">
            {[
              {label:"Chamar garçom",    desc:"Precisa de algo?",          icon:Bell,    reason:"Preciso de ajuda", clr:"text-amber-500 bg-amber-500/10"},
              {label:"Pedir a conta",    desc:"Pronto para pagar?",        icon:Receipt, reason:"Pedir conta",      clr:"text-green-500 bg-green-500/10"},
            ].map(a=>{
              const Icon=a.icon;
              return(
                <button key={a.label} onClick={()=>void callWaiter(a.reason)}
                  ...
                  <div className={`grid h-9 w-9 place-items-center rounded-xl ${a.clr}`}><Icon className="h-4 w-4"/></div>
                  <div className="flex-1"><div className="font-semibold text-sm">{a.label}</div><div className="text-xs text-muted-foreground">{a.desc}</div></div>
          ...
          {orderHistory.length>1&&(
            <div className="rounded-2xl border border-border bg-card p-4 shadow-card">
              <div className="text-xs font-bold uppercase tracking-wider text-muted-foreground mb-2">Seus pedidos desta mesa</div>
              {orderHistory.map(o=>(
              ...
              <div className="flex justify-between text-sm pt-2 border-t border-border mt-1 font-extrabold">
                <span>Total acumulado</span>
                <span className="text-primary">{fmt(orderHistory.reduce((s,o)=>s+o.total,0))}</span>
              </div>
            </div>
          )}

          <button onClick={()=>setScreen("menu")}
            className="flex h-12 w-full items-center justify-center gap-2 rounded-2xl font-bold text-white active:scale-95 transition-transform"
            style={{background:brand}}>
  ...
  return (
    <div className="min-h-screen bg-background pb-28">
      {/* Header */}
      <div className="px-5 pb-6 pt-8 text-white relative overflow-hidden" style={{background:`linear-gradient(135deg, ${brand} 0%, #111 100%)`}}>
        <div className="absolute -right-16 -top-16 h-48 w-48 rounded-full bg-white/10 blur-3xl"/>
        <div className="relative flex items-start justify-between">
          <div>
            <div className="flex items-center gap-1.5 text-xs font-bold uppercase tracking-widest text-white/60"><Utensils className="h-3.5 w-3.5"/>Mesa {table.number}</div>
            <h1 className="text-2xl font-black tracking-tight mt-0.5">{restaurant.name}</h1>
            <div className="text-sm text-white/70 mt-0.5">Olá, {customerName}!</div>
          </div>
          {lastOrderId&&(
      ...
      <div className="flex gap-2 px-4 py-2.5 border-b border-border bg-card">
        {[{l:"Chamar garçom",icon:Bell,r:"Preciso de ajuda",c:"text-amber-600 border-amber-500/30 bg-amber-500/5"},{l:"Pedir conta",icon:Receipt,r:"Pedir conta",c:"text-green-600 border-green-500/30 bg-green-500/5"}].map(a=>{
          const Icon=a.icon;
          return<button key={a.l} onClick={()=>void callWaiter(a.r)} className={`flex-1 flex items-center justify-center gap-1.5 rounded-xl border py-2.5 text-xs font-semibold ${a.c}`}><Icon className="h-4 w-4"/>{a.l}</button>;
        })}
      </div>
      ...
      <div className="sticky top-0 z-10 bg-background/95 backdrop-blur px-4 py-3 space-y-2 border-b border-border">
        ...
        {categories.length>0&&(
          <div className="flex gap-2 overflow-x-auto scrollbar-hide pb-0.5">
            {[{id:null,name:"Todos",image_url:null,sort_order:-1},...categories].map(cat=>(
              <button key={cat.id??"all"} onClick={()=>setActiveCat(cat.id??null)}
                className={`shrink-0 flex items-center gap-1.5 rounded-full px-3 py-1.5 text-xs font-semibold transition-all ${activeCat===(cat.id??null)?"text-white":"border border-border text-muted-foreground hover:bg-muted"}`}
                style={activeCat===(cat.id??null)?{background:brand}:{}}>
      ...
      <div className="px-4 py-5 space-y-6">
        ...
        {!search&&!activeCat&&featured.length>0&&(
          <section>
            <h2 className="text-xs font-bold uppercase tracking-widest text-muted-foreground mb-3 flex items-center gap-1.5"><Star className="h-3.5 w-3.5"/>Destaques</h2>
            <div className="space-y-3">{featured.map(p=><ProductCard key={p.id} product={p} onAdd={addToCart} qty={cart.find(i=>i.product.id===p.id)?.quantity??0} brand={brand}/>)}</div>
          </section>
        )}
        <section>
          {(!search&&!activeCat&&featured.length>0)&&<h2 className="text-xs font-bold uppercase tracking-widest text-muted-foreground mb-3">Cardápio completo</h2>}
          {filtered.length===0
            ?<div className="grid place-items-center py-16"><p className="text-sm text-muted-foreground">Nenhum produto encontrado.</p></div>
            :<div className="space-y-3">{filtered.map(p=><ProductCard key={p.id} product={p} onAdd={addToCart} qty={cart.find(i=>i.product.id===p.id)?.quantity??0} brand={brand}/>)}</div>}
        </section>
      </div>
        ...
        {cartCount>0&&!cartOpen&&(
          <motion.div initial={{y:100,opacity:0}} animate={{y:0,opacity:1}} exit={{y:100,opacity:0}} className="fixed bottom-6 left-4 right-4 z-20">
            <button onClick={()=>setCartOpen(true)}
              className="flex w-full items-center justify-between rounded-2xl px-5 py-4 font-bold text-white shadow-xl active:scale-95 transition-transform"
              style={{background:brand}}>
          ...
          <motion.div initial={{y:"100%"}} animate={{y:0}} exit={{y:"100%"}} transition={{type:"spring",damping:25,stiffness:300}}
            ...
            <div className="p-5 space-y-4">
                  ...
                  {noteTarget===item.product.id?(
                    <div className="flex gap-2">
                      <input autoFocus value={noteText} onChange={e=>setNoteText(e.target.value)}
                        onKeyDown={e=>{if(e.key==="Enter"){setNote(item.product.id,noteText);setNoteTarget(null);}}}
                        placeholder="Ex: sem cebola..." className="input-base flex-1 text-xs h-8"/>
                      <button onClick={()=>{setNote(item.product.id,noteText);setNoteTarget(null);}} className="h-8 px-3 rounded-lg gradient-brand text-xs text-primary-foreground font-bold">OK</button>
                    </div>
                  ):(
                    <button onClick={()=>{setNoteTarget(item.product.id);setNoteText(item.notes);}} className="text-[11px] text-muted-foreground hover:text-foreground transition-colors">
              ...
              <button onClick={()=>void sendOrder()} disabled={sending}
                className="flex h-14 w-full items-center justify-center gap-2 rounded-2xl font-extrabold text-white active:scale-95 transition-transform"
                style={{background:brand}}>
...
function ProductCard({product,onAdd,qty,brand}:{product:Product;onAdd:(p:Product)=>void;qty:number;brand:string}){
  const price=product.promo_price??product.price;
  return(
    <div className="flex items-center gap-3 rounded-2xl border border-border bg-card p-3 shadow-card active:scale-[0.99] transition-transform">
      {product.image_url
      ...
      <div className="flex-1 min-w-0">
        ...
        <div className="mt-1.5 flex items-center gap-2 flex-wrap">
          <span className="font-extrabold text-sm" style={{color:brand}}>{fmt(price)}</span>
          {product.promo_price&&<span className="text-xs text-muted-foreground line-through">{fmt(product.price)}</span>}
      ...
      <button onClick={()=>onAdd(product)} className="grid h-9 w-9 shrink-0 place-items-center rounded-xl text-white shadow-sm active:scale-95 transition-transform" style={{background:brand}}>
